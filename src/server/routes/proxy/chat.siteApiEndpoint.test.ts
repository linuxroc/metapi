...
describe('chat proxy site api endpoint rotation', () => {
  ...
  it('rotates to the next configured ai endpoint for retryable /v1/chat/completions failures', async () => {
    ...
    expect(String(fetchMock.mock.calls[2]?.[0] || '')).toBe('https://api-b.example.com/v1/chat/completions');
    ...
    expect(storedEndpoints[0]).toMatchObject({
      url: 'https://api-a.example.com',
      lastFailureReason: 'HTTP 502: [upstream:/v1/responses] Upstream returned HTTP 502: bad gateway via responses',
    });
